...
const WEB_FLOW = `<svg viewBox="0 0 720 280" xmlns="http://www.w3.org/2000/svg" role="img">
  ...
  ${bi(441, 192, "loaded only if you edit", "solo si usted edita", "sub")}

  <rect class="bx-warn" x="20" y="230" width="680" height="40"/>
  ${bi(36, 255, "A number on screen is never computed on load: it is replayed from a committed artifact", "Un numero en pantalla nunca se calcula al cargar: se reproduce desde un artefacto versionado")}
</svg>`;
...
export const ArchitectureTabs: ArchTab[] = [
  ...
  },
  {
    ...
    es: "El flujo web",
    body_en:
      "A static page with no backend. Nothing is computed when the page loads: the case file is fetched rather than bundled, so the first paint does not wait on 220 kilobytes of JSON, and every figure shown is replayed from what the bake committed.\n\nThe shell, the header, the footer, the theme and the language toggle all come from a package shared across this line of products, so they are identical by construction and a fix lands once rather than in every app.\n\nThe workbench shows one case at a time. That is deliberate: a cross-case summary answers 'across all cases' and belongs on Experiments or Benchmark, while the workbench answers 'what happened here'.\n\nThe solver only loads if you edit something. A reader who never touches a case never downloads it.",
    body_es:
      "Una pagina estatica sin servidor. Nada se calcula al cargar: el archivo de casos se descarga en vez de empaquetarse, de modo que el primer render no espera 220 kilobytes de JSON, y cada cifra mostrada se reproduce de lo que el calculo dejo versionado.\n\nEl shell, el encabezado, el pie, el tema y el selector de idioma vienen de un paquete compartido por esta linea de productos, asi que son identicos por construccion y una correccion se aplica una sola vez.\n\nEl banco de trabajo muestra un caso a la vez. Es deliberado: un resumen entre casos responde 'en todos los casos' y pertenece a Experimentos o Comparativa, mientras que el banco responde 'que paso aqui'.\n\nEl solver solo se carga si usted edita algo. Un lector que nunca toca un caso nunca lo descarga.",
    svg: WEB_FLOW,
  },
  {
    id: "contracts",
